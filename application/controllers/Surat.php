...
// surat keluar keluar
		...
		if ($this->input->post('id') != null ) {
			...
			if (empty($foto)){
				$data = array(
					...
					'no_surat' => $no_surat,
					'tujuan' => $tujuan,
					'tgl_catat' => $tanggal_diterima,
					'tgl_surat' => $tanggal_surat,
					'kode_klasifikasi' => $klasifikasi,
					'id_index' => $index_surat,
					'isi' => $isi_ringkasan,
					'keterangan' => $keterangan,
				 );
			}else {
			...
			$this->db->where('id_suratkeluar', $this->input->post('id'));
			$this->db->update('tb_suratkeluar', $data);
			$this->session->set_flashdata('pesan', '<div class="alert alert-success">Berhasil Menambah Data Surat Keluar.!</div>');
	...
	public function esuratkeluar($id_suratkeluar)
	{
		...
		$row =  $this->Model_surat->edsuratkeluar($where);
		...
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('backend/surat/keluar/tambah',$data);
		$this->load->view('templates/footer');
	}
